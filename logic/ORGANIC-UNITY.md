...
- **Entity (Empirical)**: Instance in FormDB (Neo4j)
  - References Form via `formId`
  - Has `facets` and `signature` (JSON strings) for Organic Unity
  - Bridges to Model/Prisma via facets
  - Stores no empirical values (those are in Model/Postgres)
...
- Entity = Empirical instance (what is)
