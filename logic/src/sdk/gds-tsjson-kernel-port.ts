...
function tryParseGdsModelId(
  modelId: string,
): { facade: string; op: string } | undefined {
  const parts = modelId
    .split('.')
    .map((p) => p.trim())
    .filter(Boolean);
...
function kernelRequestToGdsApplicationCall(
  request: KernelRunRequest,
): GdsApplicationCall {
  if (
    isPlainObject(request.input) &&
    typeof request.input.facade === 'string' &&
    typeof request.input.op === 'string'
  ) {
  ...
  if (!isPlainObject(request.input)) {
    throw new Error(
      'KernelRunRequest.input must be an object when using gds.<facade>.<op> model ids',
    );
  }
