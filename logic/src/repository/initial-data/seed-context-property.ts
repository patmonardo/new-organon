...
export async function seedContextProperty() {
  ...
  try {
    ...
    const formCheck = await session.run("MATCH (f:FormShape) RETURN count(f) as count");
    const entityCheck = await session.run("MATCH (e:Entity) RETURN count(e) as count");
    ...
    console.log(`   Found ${formCount} forms and ${entityCount} entities to reflect upon\n`);
    ...
    const invoiceContext: ContextShape = {
      core: {
    ...
    const productContext: ContextShape = {
      core: {
    ...
    const invoiceCustomerProperty: PropertyShape = {
        ...
        law: {
          invariants: [
            ...
            },
            {
              ...
              predicate: "exists(customer) && customer.createdAt < invoice.createdAt",
              universality: "necessary",
            },
          ],
          universality: "necessary",
          ...
          evidence: [
            {
              ...
              definition: "Invoice entities link to Customer entities via BELONGS_TO",
              type: "relationship",
            },
          ],
        },
    ...
    const productInventoryProperty: PropertyShape = {
          ...
          evidence: [
            {
              ...
              definition: "Product entities maintain stock and pricing information",
              type: "entity",
            },
          ],
        },
    ...
    const invoiceStatusProperty: PropertyShape = {
      ...
      facets: {
        law: {
          invariants: [
            {
              id: "inv-1",
              constraint: "Invoice status transitions: DRAFT → PENDING → PAID/OVERDUE",
