...
describe.skipIf(!neo4jOk)('ShapeEngine × Neo4j (createFormDb)', () => {
  it('persists create/setCore/delete through FormShapeRepository', async () => {
    const { shapeEngine: engine, shapeRepo: repo } = createFormDb({ connection: defaultConnection });
    ...
    try {
      ...
      expect(saved1?.shape.core.id).toBe(id);
      ...
      expect(saved2?.shape.core.name).toBe('Neo4j Form v2');
