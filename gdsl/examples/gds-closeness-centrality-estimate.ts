/**
 * Example: Closeness Centrality algorithm in estimate mode with memory submode
 *
 * Demonstrates running Closeness Centrality in estimate mode with memory submode to get memory estimation.
 */
...
export function closenessCentralityEstimateDemo(): void {
  const user = { username: 'alice', isAdmin: true };
  const databaseId = 'db1';
  const graphName = `closeness-estimate-${Date.now()}`;
  ...
  console.log('Closeness Centrality Estimate Request:');
  // eslint-disable-next-line no-console
  ...
  // eslint-disable-next-line no-console
  console.log('\nCloseness Centrality Estimate Response:');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  closenessCentralityEstimateDemo();
}
