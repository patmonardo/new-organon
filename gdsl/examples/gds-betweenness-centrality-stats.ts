/**
 * Example: Betweenness Centrality algorithm in stats mode
 *
 * Demonstrates running Betweenness Centrality in stats mode to get execution statistics.
 */
...
export function betweennessCentralityStatsDemo(): void {
  const user = { username: 'alice', isAdmin: true };
  const databaseId = 'db1';
  const graphName = `betweenness-stats-${Date.now()}`;
  ...
  console.log('Betweenness Centrality Stats Request:');
  // eslint-disable-next-line no-console
  ...
  // eslint-disable-next-line no-console
  console.log('\nBetweenness Centrality Stats Response:');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  betweennessCentralityStatsDemo();
}
