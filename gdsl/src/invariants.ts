...
import { KernelRunResultSchema } from './kernel-api';
import { FACT_STORE_MODES, type FactStoreOp } from './sdk/terminology';
 ...
 */
export const KernelRunResultStrictSchema = KernelRunResultSchema.superRefine((result, ctx) => {
  if (result.ok) {
    if (result.error !== undefined) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'KernelRunResult invariant violated: ok=true requires error to be undefined',
      });
    }
  } else {
    if (result.error === undefined) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'KernelRunResult invariant violated: ok=false requires error to be defined',
      });
    }
  }
});
