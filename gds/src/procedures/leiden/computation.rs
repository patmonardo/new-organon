...
where F: Fn(usize) -> Vec<(usize, f64)>
{
    ...
    let node_volumes: Vec<f64> = (0..node_count).map(|n| get_neighbors(n).iter().map(|(_, w)| w).sum()).collect();
    ...
    for level in 0..config.max_iterations {
        ...
        for (i, &c) in communities.iter().enumerate() { *cvol.entry(c).or_insert(0.0) += node_volumes[i]; }
        ...
        while changed && iter < 100 {
            ...
            for node in 0..node_count {
                ...
                for (&cd, &ew) in &cw {
                    ...
                    let g = (ew - wc) / total_volume - config.gamma * nv * (cdv - cvwo) / (total_volume * total_volume);
                    if g > bg { bg = g; bc = cd; }
                }
                if bc != cc && bg > 1e-10 {
                    communities[node] = bc;
        ...
        for n in 0..node_count {
            for (nb, w) in get_neighbors(n) {
                if communities[nb] == communities[n] { ie += w; }
            }
        }
        let mut m = ie / total_volume;
        for v in cvol.values() { m -= config.gamma * (v * v) / (total_volume * total_volume); }
