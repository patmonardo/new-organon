...
impl LocalClusteringCoefficientFacade {
    ...
    pub fn estimate_memory(&self) -> Result<MemoryRange> {
        // Local clustering coefficient stores one f64 per node and uses neighbor traversal.
        let node_count = self.graph_store.node_count();
        let relationship_count = self.graph_store.relationship_count();

        // Per node: coefficient (f64) + some counters.
        let per_node = 64usize;
        // Per relationship: temporary neighbor iteration state.
