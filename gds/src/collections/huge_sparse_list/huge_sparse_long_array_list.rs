...
/// A long-indexable mutable sparse list of Vec<i64> that can contain more than 2 billion elements.
///
/// Uses a HashMap to track pages, only allocating memory for pages that contain values.
/// Perfect for dynamic sparse collections of long arrays that grow and change over time.
///
/// **Memory Efficiency**: Only allocates 4KB pages where Vec<i64> values actually exist.
...
/// **Default Values**: Returns a clone of the default Vec<i64> for unset indices.
///
/// # Examples
///
/// ```
...
///
/// list.set(0, vec![1, 2, 3]);
/// list.set(1_000_000, vec![4, 5]);
///
/// assert_eq!(list.get(0), vec![1, 2, 3]);
/// assert_eq!(list.get(1_000_000), vec![4, 5]);
/// assert_eq!(list.get(50), Vec::<i64>::new()); // default
/// ```
#[derive(Debug)]
pub struct HugeSparseLongArrayList {
    /// Maximum capacity (highest index + 1)
    capacity: RefCell<usize>,
    /// Default Vec<i64> returned for unset indices
    default_value: Vec<i64>,
    /// HashMap storing only allocated pages
    pages: RefCell<HashMap<usize, Vec<Vec<i64>>>>,
    /// HashSet tracking which indices have been explicitly set
    set_indices: RefCell<HashSet<usize>>,
}

impl HugeSparseLongArrayList {
    /// Create a new sparse long array list with the specified default value.
    ///
    /// # Arguments
    ///
    /// * `default_value` - The Vec<i64> returned for unset indices
    ///
    /// # Examples
    ///
    /// ```
    /// use gds::collections::HugeSparseLongArrayList;
    ///
    /// let list = HugeSparseLongArrayList::of(vec![0]);
    /// list.set(42, vec![1, 2, 3]);
    /// assert_eq!(list.get(42), vec![1, 2, 3]);
    /// ```
    pub fn of(default_value: Vec<i64>) -> Self {
        Self::with_capacity(default_value, 0)
    }

    /// Create a new sparse long array list with an initial capacity hint.
    ///
    ...
    pub fn with_capacity(default_value: Vec<i64>, capacity: usize) -> Self {
        Self {
    ...
    /// Get the Vec<i64> at the given index.
    ///
    /// Returns a reference to the default Vec<i64> if the index was never set.
    ///
    /// # Arguments
    ///
    ...
    /// Set the Vec<i64> value at the given index.
    ///
    ...
    /// * `value` - The Vec<i64> to store
    pub fn set(&self, index: usize, value: Vec<i64>) {
    ...
    /// # Arguments
    ///
    /// * `consumer` - Callback function receiving (index, &Vec<i64>) pairs
    ...
    /// Note: Does not account for heap allocations within Vec<i64> elements.
    pub fn size_of(&self) -> usize {
        let page_size = PAGE_SIZE * std::mem::size_of::<Vec<i64>>();
...
mod tests {
    ...
    fn test_basic_operations() {
        ...
        list.set(0, vec![1, 2, 3]);
        list.set(1, vec![4, 5]);
        list.set(100, vec![6, 7, 8, 9]);

        assert_eq!(list.get(0), vec![1, 2, 3]);
        assert_eq!(list.get(1), vec![4, 5]);
        assert_eq!(list.get(100), vec![6, 7, 8, 9]);
        assert_eq!(list.get(50), Vec::<i64>::new()); // default
    }
    ...
    fn test_sparse_distribution() {
        ...
        list.set(0, vec![1]);
        list.set(10_000_000, vec![2]);
        list.set(50_000_000, vec![3]);
        list.set(100_000_000, vec![4]);
        ...
        // Verify values
        assert_eq!(list.get(0), vec![1]);
        assert_eq!(list.get(10_000_000), vec![2]);
        assert_eq!(list.get(50_000_000), vec![3]);
        assert_eq!(list.get(100_000_000), vec![4]);
    }
    ...
    fn test_empty_vectors() {
        ...
        assert_eq!(list.get(0), Vec::<i64>::new()); // explicitly set to empty
        assert_eq!(list.get(1), vec![1, 2]);
        assert_eq!(list.get(2), vec![0]); // default
    ...
    fn test_default_value() {
        let default = vec![9, 8, 7];
        let list = HugeSparseLongArrayList::of(default.clone());
        list.set(42, vec![1, 2]);

        assert_eq!(list.get(42), vec![1, 2]);
    ...
    fn test_capacity_growth() {
        ...
        list.set(5, vec![3]); // shouldn't shrink
        assert_eq!(list.capacity(), 1001);
    }
    ...
    fn test_page_boundaries() {
        ...
        list.set(PAGE_SIZE - 1, vec![1]); // last element of first page
        list.set(PAGE_SIZE, vec![2]); // first element of second page
        list.set(PAGE_SIZE + 1, vec![3]); // second element of second page

        assert_eq!(list.get(PAGE_SIZE - 1), vec![1]);
        assert_eq!(list.get(PAGE_SIZE), vec![2]);
        assert_eq!(list.get(PAGE_SIZE + 1), vec![3]);
    ...
    fn test_overwrite() {
        let list = HugeSparseLongArrayList::of(vec![]);
        list.set(42, vec![1, 2]);
        list.set(42, vec![3, 4, 5]); // overwrite

        assert_eq!(list.get(42), vec![3, 4, 5]);
    }
    ...
    fn test_for_all() {
        ...
        assert_eq!(
            collected,
            vec![(0, vec![1, 2]), (100, vec![3, 4]), (1000, vec![5, 6])]
        );
    }
    ...
    fn test_large_vectors() {
        ...
        // Store large vectors
        let large_vec: Vec<i64> = (0..10_000).collect();
    ...
    fn test_with_capacity() {
        let list = HugeSparseLongArrayList::with_capacity(vec![], 1000);
        assert_eq!(list.capacity(), 1000);

        list.set(500, vec![1, 2, 3]);
        assert_eq!(list.capacity(), 1000);
        assert_eq!(list.get(500), vec![1, 2, 3]);
    }
}
