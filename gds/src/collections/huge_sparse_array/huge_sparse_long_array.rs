...
mod tests {
    ...
    fn test_sparse_distribution() {
        ...
        builder.set(0, 1);
        builder.set(PAGE_SIZE + 10, 2);
        builder.set(PAGE_SIZE * 2 + 100, 3);
        builder.set(PAGE_SIZE * 3 + 1000, 4);

        let array = builder.build();

        assert_eq!(array.get(0), 1);
        assert_eq!(array.get(PAGE_SIZE + 10), 2);
        assert_eq!(array.get(PAGE_SIZE * 2 + 100), 3);
        assert_eq!(array.get(PAGE_SIZE * 3 + 1000), 4);
    ...
    fn test_page_boundaries() {
        ...
        builder.set(PAGE_SIZE - 1, 100); // Last element of page 0
        builder.set(PAGE_SIZE, 200); // First element of page 1
        builder.set(PAGE_SIZE + 1, 300); // Second element of page 1

        let array = builder.build();

        assert_eq!(array.get(PAGE_SIZE - 1), 100);
        assert_eq!(array.get(PAGE_SIZE), 200);
        assert_eq!(array.get(PAGE_SIZE + 1), 300);
