...
pub trait ComputeKernels<T>: Collections<T> {
    ...
    fn simd_min(&self) -> Option<T>
    where
        T: Ord;

    /// SIMD-accelerated max operation
    fn simd_max(&self) -> Option<T>
    where
        T: Ord;

    /// SIMD-accelerated standard deviation
    fn simd_std_dev(&self) -> Option<f64>
    where
        T: Into<f64>;

    /// SIMD-accelerated variance
    fn simd_variance(&self) -> Option<f64>
    where
        T: Into<f64>;
    ...
    fn sort_in_place(&mut self) -> Result<(), ComputeError>
    where
        T: Ord;

    /// Unique values (returns new collection)
    fn unique(&self) -> Result<Self, ComputeError>
    where
        Self: Sized,
        T: Ord + Clone;
...
pub trait MLComputeKernels<T>: ComputeKernels<T> {
    ...
    /// Dot product
    fn dot(&self, other: &Self) -> Result<T, ComputeError>
    where
        T: Sum;
