...
impl GraphStoreValidationService {
    ...
    pub fn ensure_relationships_may_be_deleted<G: GraphStore>(
        ...
        if !self.has_relationship_type(graph_store, relationship_type) {
            return Err(format!(
                "Relationship type '{}' does not exist in graph '{}'",
                relationship_type, graph_name
            ));
        }
        Ok(())
    }
        ...
        for label in node_labels {
            for property in node_properties {
                if !self.has_node_property_for_label(graph_store, label, property) {
                    return Err(format!(
                        "Node property '{}' does not exist for label '{}'",
                        property, label
                    ));
                }
            }
        ...
        for rel_type in relationship_types {
            for property in relationship_properties {
                if !self.has_relationship_property_for_type(graph_store, rel_type, property) {
                    return Err(format!(
                        "Relationship property '{}' does not exist for type '{}'",
                        property, rel_type
                    ));
                }
            }
