...
impl ExportToCsvApplication {
	pub fn compute(&self, graph_store: &DefaultGraphStore, export_path: &str) -> Result<ExportResult, String> {
		...
		fs::create_dir_all(&export_root)
			.map_err(|e| format!("Failed to create export directory '{}': {e}", export_root.display()))?;
	...
	fn write_nodes_csv(&self, graph_store: &DefaultGraphStore, export_root: &Path) -> Result<u64, String> {
	...
	fn write_relationships_for_type(
		...
		let filtered = graph
			...
			.map_err(|e| format!("Failed to filter graph by relationship type '{}': {e}", rel_type.name()))?;
		...
		for mapped_source in 0..node_count {
			...
			for cursor in filtered.stream_relationships(mapped_source, filtered.default_property_value()) {
