...
use crate::applications::graph_store_catalog::facade::{GenericProjectionConfig, ProjectionResult};
use crate::core::User;
...
///
/// Pass-1 implementation mirrors `NativeProjectApplication`:
/// - clone from catalog when `source_graph_name` provided
/// - otherwise optionally generate a small deterministic fictitious store
#[derive(Clone, Debug, Default)]
pub struct GenericProjectApplication;

impl GenericProjectApplication {
    pub fn project(
        &self,
        graph_store_catalog_service: Arc<dyn GraphStoreCatalogService>,
        user: &dyn User,
        database_id: &DatabaseId,
        cfg: &GenericProjectionConfig,
        ...
        let mut store: DefaultGraphStore = match cfg.source_graph_name.as_deref() {
            ...
            None => {
                if !cfg.fictitious_loading {
                    return Err(
                        "project_generic requires projectionConfig.sourceGraphName (or set fictitiousLoading=true)"
                            .to_string(),
                    );
                }
        ...
        // Minimal “projection vocabulary” (pass-1): nodeLabels validation.
