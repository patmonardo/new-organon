...
fn get_bool(request: &Value, key: &str, default: bool) -> bool {
	request.get(key).and_then(|v| v.as_bool()).unwrap_or(default)
}
...
pub fn handle_graph_store_catalog_dispatch(
	...
	match operation {
		...
		"dropNodeProperties" => handle_drop_node_properties(operation, request, catalog_apps, user, db),
		"dropRelationships" => handle_drop_relationships(operation, request, catalog_apps, user, db),
		"dropGraphProperty" => handle_drop_graph_property(operation, request, catalog_apps, user, db),
		"streamGraphProperty" => handle_stream_graph_property(operation, request, catalog_apps, user, db),
		"graphMemoryUsage" => handle_graph_memory_usage(operation, request, catalog_apps, user, db),
		"streamNodeProperties" => handle_stream_node_properties(operation, request, catalog_apps, user, db),
		"streamRelationshipProperties" => handle_stream_relationship_properties(operation, request, catalog_apps, user, db),
		"streamRelationships" => handle_stream_relationships(operation, request, catalog_apps, user, db),
		"generateGraph" => handle_generate_graph(operation, request, catalog_apps, user, db),
		"generateGraphStats" => handle_generate_graph_stats(operation, request, catalog_apps, user, db),
		"exportToCsvEstimate" => handle_export_to_csv_estimate(operation, request, catalog_apps, user, db),
		...
		"sampleCommonNeighbourAwareRandomWalk" => {
			handle_sample_common_neighbour_aware_random_walk(operation, request, catalog_apps, user, db)
		}
		"estimateCommonNeighbourAwareRandomWalk" => {
			handle_estimate_common_neighbour_aware_random_walk(operation, request, catalog_apps, user, db)
		}
		"estimateNativeProject" => handle_estimate_native_project(operation, request, catalog_apps, user, db),
		"cypherProject" => handle_cypher_project(operation, request, catalog_apps, user, db),
		"estimateCypherProject" => handle_estimate_cypher_project(operation, request, catalog_apps, user, db),
...
fn handle_generate_graph(
	...
	let generation_config: GraphGenerationConfig = match parse_required(op, request, "generationConfig") {
		Ok(v) => v,
		Err(e) => return e,
	};
...
fn handle_estimate_common_neighbour_aware_random_walk(
	...
	match catalog_apps.estimate_common_neighbour_aware_random_walk(user, db, graph_name, &configuration)
	{
		Ok(result) => ok(op, json!(result)),
		Err(msg) => err(op, "ERROR", msg),
	}
...
fn handle_estimate_native_project(
	...
	let projection_config: NativeProjectionConfig = match parse_required(op, request, "projectionConfig") {
		Ok(cfg) => cfg,
		Err(e) => return e,
	};
...
fn handle_cypher_project(
	...
	match catalog_apps.project_cypher(user, db, graph_name, node_query, relationship_query, &configuration)
	{
		Ok(result) => ok(op, json!(result)),
		Err(msg) => err(op, "ERROR", msg),
	}
...
fn handle_estimate_cypher_project(
	...
	match catalog_apps.estimate_project_cypher(user, db, node_query, relationship_query, &configuration)
	{
		Ok(result) => ok(op, json!(result)),
		Err(msg) => err(op, "ERROR", msg),
	}
