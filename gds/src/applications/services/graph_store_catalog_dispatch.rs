...
pub fn handle_graph_store_catalog_dispatch(
    ...
    match op {
        "graph_exists" => {
            let graph_name = request.get("graphName").and_then(|v| v.as_str()).unwrap_or("");
        ...
        "drop_graph" => {
            let graph_name = request.get("graphName").and_then(|v| v.as_str()).unwrap_or("");
            let fail_if_missing = request.get("failIfMissing").and_then(|v| v.as_bool()).unwrap_or(false);
            match catalog_apps.drop_graph(user, db, graph_name, fail_if_missing) {
                Ok(e) => json!({ "ok": true, "op": op, "data": { "dropped": [ { "name": e.graph_name(), "nodeCount": e.node_count(), "relationshipCount": e.relationship_count() } ] } }),
                Err(msg) => json!({ "ok": false, "op": op, "error": { "code": "ERROR", "message": msg } }),
            }
        }
        "drop_graphs" => {
            let names = request
            ...
            match catalog_apps.drop_graphs(user, db, &names, fail_if_missing) {
                Ok(v) => json!({ "ok": true, "op": op, "data": { "dropped": v.iter().map(|e| json!({ "name": e.graph_name(), "nodeCount": e.node_count(), "relationshipCount": e.relationship_count() })).collect::<Vec<_>>() } }),
                Err(msg) => json!({ "ok": false, "op": op, "error": { "code": "ERROR", "message": msg } }),
            }
        }
        "drop_node_properties" => {
            ...
            match catalog_apps.drop_node_properties(user, db, graph_name, &node_properties, fail_if_missing) {
                Ok(count) => json!({ "ok": true, "op": op, "data": { "propertiesRemoved": count } }),
                Err(msg) => json!({ "ok": false, "op": op, "error": { "code": "ERROR", "message": msg } }),
            }
        }
        "drop_relationships" => {
            let graph_name = request.get("graphName").and_then(|v| v.as_str()).unwrap_or("");
            let relationship_type = request.get("relationshipType").and_then(|v| v.as_str()).unwrap_or("");
            match catalog_apps.drop_relationships(user, db, graph_name, relationship_type) {
                Ok(result) => json!({ "ok": true, "op": op, "data": { "relationshipsRemoved": result.deleted_relationship_count().unwrap_or(0) } }),
                Err(msg) => json!({ "ok": false, "op": op, "error": { "code": "ERROR", "message": msg } }),
            }
        }
        "graph_memory_usage" => {
        ...
        "stream_node_properties" => {
            ...
            let node_labels = request
                ...
                .map(|arr| arr.iter().filter_map(|x| x.as_str().map(String::from)).collect::<Vec<_>>())
                .unwrap_or_default();
            let list_node_labels = request.get("listNodeLabels").and_then(|v| v.as_bool()).unwrap_or(false);

            match catalog_apps.stream_node_properties(user, db, graph_name, &node_properties, &node_labels, list_node_labels) {
                Ok(results) => json!({ "ok": true, "op": op, "data": { "results": results } }),
                Err(msg) => json!({ "ok": false, "op": op, "error": { "code": "STREAM_FAILED", "message": msg } }),
            }
        }
        "stream_relationship_properties" => {
            ...
            match catalog_apps.stream_relationship_properties(user, db, graph_name, &relationship_properties, &relationship_types) {
                Ok(results) => json!({ "ok": true, "op": op, "data": { "results": results } }),
                Err(msg) => json!({ "ok": false, "op": op, "error": { "code": "STREAM_FAILED", "message": msg } }),
            }
        }
        ...
        "generate_graph" => {
            ...
            let generation_config = crate::applications::graph_store_catalog::facade::GraphGenerationConfig;
            match catalog_apps.generate_graph(user, db, &generation_config) {
                Ok(result) => json!({ "ok": true, "op": op, "data": { "graphName": result.graph_name(), "nodesGenerated": result.nodes_generated(), "relationshipsGenerated": result.relationships_generated(), "generationTimeMs": result.generation_time_ms() } }),
                Err(msg) => json!({ "ok": false, "op": op, "error": { "code": "GENERATION_FAILED", "message": msg } }),
            }
        }
        "sample_graph" => {
            let graph_name = request.get("graphName").and_then(|v| v.as_str()).unwrap_or("");
            ...
            match catalog_apps.sample_graph(user, db, graph_name, &sampling_config) {
                Ok(result) => json!({ "ok": true, "op": op, "data": { "sampledGraphName": result.sampled_graph_name(), "originalNodes": result.original_nodes(), "sampledNodes": result.sampled_nodes(), "originalRelationships": result.original_relationships(), "sampledRelationships": result.sampled_relationships() } }),
                Err(msg) => json!({ "ok": false, "op": op, "error": { "code": "SAMPLING_FAILED", "message": msg } }),
            }
        }
        _ => json!({ "ok": false, "op": op, "error": { "code": "UNSUPPORTED_OP", "message": "Unsupported graph_store_catalog operation." } }),
    }
}
