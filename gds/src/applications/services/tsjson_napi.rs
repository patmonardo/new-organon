...
fn handle_algorithms(request: &serde_json::Value) -> serde_json::Value {
    ...
    match op {
        ...
        // ============================================================================
        // Legacy *_stream handlers (backward compatibility, deprecated)
        // ============================================================================
        "bfs_stream" => {
            // --- BFS Stream via Applications Machinery Template ---
            // DEPRECATED: Use op: "bfs" with mode: "stream" instead
            ...
            // Process through the machinery template
            let result = template.process(
                catalog.as_ref(),
                graph_name,
                &cfg,
                Algorithm::BFS,
                // Compute closure: run BFS on the loaded graph
                |resources| {
                    let graph = resources.facade();
                    let mut builder = graph.bfs().source(source_node);
                    if !targets.is_empty() {
                ...
                // Render closure: transform to JSON rows
                |stream_result| {
            ...
            let rows = match result {
                Ok(Ok(rows)) => rows,
                Ok(Err(e)) => return err(op, "ERROR", &format!("BFS failed: {e}")),
                Err(e) => return err(op, "ERROR", &format!("Processing failed: {e}")),
            };
        ...
        "dfs_stream" => {
            // --- DFS Stream via Applications Machinery Template ---
            // Same pattern as BFS: routes through StreamProcessingTemplate for Java parity.
            use crate::applications::algorithms::machinery::StreamProcessingTemplate;
            use crate::applications::algorithms::metadata::Algorithm;
            use crate::config::base_types::AlgoBaseConfig;
            ...
            let result = template.process(
                ...
                &cfg,
                Algorithm::DFS,
                // Compute closure: run DFS on the loaded graph
                |resources| {
                    let graph = resources.facade();
                    let mut builder = graph.dfs().source(source_node);
                    if !targets.is_empty() {
                        builder = builder.targets(targets.clone());
                    }
                    if let Some(d) = max_depth {
            ...
            let rows = match result {
                Ok(Ok(rows)) => rows,
                Ok(Err(e)) => return err(op, "ERROR", &format!("DFS failed: {e}")),
                Err(e) => return err(op, "ERROR", &format!("Processing failed: {e}")),
            };
        ...
        "dijkstra_stream" => {
            // --- Dijkstra Stream via Applications Machinery Template ---
            use crate::applications::algorithms::machinery::StreamProcessingTemplate;
            use crate::applications::algorithms::metadata::Algorithm;
            use crate::config::base_types::AlgoBaseConfig;
            ...
            let result = template.process(
                ...
                &cfg,
                Algorithm::Dijkstra,
                // Compute closure: run Dijkstra on the loaded graph
                |resources| {
                    let graph = resources.facade();
            ...
            let rows = match result {
                Ok(Ok(rows)) => rows,
                Ok(Err(e)) => return err(op, "ERROR", &format!("Dijkstra failed: {e}")),
                Err(e) => return err(op, "ERROR", &format!("Processing failed: {e}")),
            };
        ...
        "bellman_ford_stream" => {
            // --- Bellman-Ford Stream via Applications Machinery Template ---
            ...
            let result = template.process(
                ...
                // Compute closure: run Bellman-Ford on the loaded graph
                |resources| {
                    let graph = resources.facade();
            ...
            let rows = match result {
                Ok(Ok(rows)) => rows,
                Ok(Err(e)) => return err(op, "ERROR", &format!("Bellman-Ford failed: {e}")),
                Err(e) => return err(op, "ERROR", &format!("Processing failed: {e}")),
            };
        ...
        "astar_stream" => {
            // --- A* Stream via Applications Machinery Template ---
            ...
            let result = template.process(
                ...
                &cfg,
                Algorithm::AStar,
                // Compute closure: run A* on the loaded graph
                |resources| {
                    let graph = resources.facade();
            ...
            let rows = match result {
                Ok(Ok(rows)) => rows,
                Ok(Err(e)) => return err(op, "ERROR", &format!("A* failed: {e}")),
                Err(e) => return err(op, "ERROR", &format!("Processing failed: {e}")),
            };
        ...
        "delta_stepping_stream" => {
            // --- Delta Stepping Stream via Applications Machinery Template ---
            // Parallel SSSP using binning strategy
            use crate::applications::algorithms::machinery::StreamProcessingTemplate;
            use crate::applications::algorithms::metadata::Algorithm;
            use crate::config::base_types::AlgoBaseConfig;
            ...
            let rows = match result {
                Ok(Ok(rows)) => rows,
                Ok(Err(e)) => return err(op, "ERROR", &format!("Delta Stepping failed: {e}")),
                Err(e) => return err(op, "ERROR", &format!("Processing failed: {e}")),
            };
        ...
        "yens_stream" => {
            // --- Yen's K Shortest Paths Stream via Applications Machinery Template ---
            use crate::applications::algorithms::machinery::StreamProcessingTemplate;
            use crate::applications::algorithms::metadata::Algorithm;
            use crate::config::base_types::AlgoBaseConfig;
            ...
            let rows = match result {
                Ok(Ok(rows)) => rows,
                Ok(Err(e)) => return err(op, "ERROR", &format!("Yen's failed: {e}")),
                Err(e) => return err(op, "ERROR", &format!("Processing failed: {e}")),
            };
        ...
        "all_shortest_paths_stream" => {
            // --- All Shortest Paths Stream via Applications Machinery Template ---
            use crate::applications::algorithms::machinery::StreamProcessingTemplate;
            use crate::applications::algorithms::metadata::Algorithm;
            use crate::config::base_types::AlgoBaseConfig;
            ...
            let rows = match result {
                Ok(Ok(rows)) => rows,
                Ok(Err(e)) => return err(op, "ERROR", &format!("All Shortest Paths failed: {e}")),
                Err(e) => return err(op, "ERROR", &format!("Processing failed: {e}")),
            };
        ...
        "spanning_tree_stream" => {
            // --- Spanning Tree (Prim) Stream via Applications Machinery Template ---
            use crate::applications::algorithms::machinery::StreamProcessingTemplate;
            use crate::applications::algorithms::metadata::Algorithm;
            use crate::config::base_types::AlgoBaseConfig;
            ...
            let rows = match result {
                Ok(Ok(rows)) => rows,
                Ok(Err(e)) => return err(op, "ERROR", &format!("Spanning Tree failed: {e}")),
                Err(e) => return err(op, "ERROR", &format!("Processing failed: {e}")),
            };
        ...
        "topological_sort_stream" => {
            // --- Topological Sort Stream via Applications Machinery Template ---
            use crate::applications::algorithms::machinery::StreamProcessingTemplate;
            use crate::applications::algorithms::metadata::Algorithm;
            use crate::config::base_types::AlgoBaseConfig;
            ...
            let rows = match result {
                Ok(Ok(rows)) => rows,
                Ok(Err(e)) => return err(op, "ERROR", &format!("Topological Sort failed: {e}")),
                Err(e) => return err(op, "ERROR", &format!("Processing failed: {e}")),
            };
        ...
        "random_walk_stream" => {
            // --- Random Walk Stream via Applications Machinery Template ---
            use crate::applications::algorithms::machinery::StreamProcessingTemplate;
            use crate::applications::algorithms::metadata::Algorithm;
            use crate::config::base_types::AlgoBaseConfig;
            ...
            let rows = match result {
                Ok(Ok(rows)) => rows,
                Ok(Err(e)) => return err(op, "ERROR", &format!("Random Walk failed: {e}")),
                Err(e) => return err(op, "ERROR", &format!("Processing failed: {e}")),
            };
        ...
        "pagerank_stream" => {
            ...
            let catalog = TSJSON_CATALOG_SERVICE.clone().graph_catalog(&user, &db);
            let store = match catalog.get(graph_name) {
                Some(s) => s,
                None => return err(op, "NOT_FOUND", "Graph not found"),
            };

            let has_wildcard = relationship_types.iter().any(|s| s.trim() == "*");
            // Relationship property selection (for weights): weightProperty applies to all types unless overridden
            // by relationshipPropertySelectors.
            let weight_property = request
                .get("weightProperty")
                .and_then(|v| v.as_str())
                .map(|s| s.to_string());
            let relationship_property_selectors: std::collections::HashMap<String, String> = request
                .get("relationshipPropertySelectors")
                .and_then(|v| v.as_object())
                .map(|obj| {
                    obj.iter()
            ...
            let graph = if relationship_types.is_empty() || has_wildcard {
                store.get_graph()
            } else {
                let set: std::collections::HashSet<RelationshipType> = relationship_types
                    .iter()
                    .filter_map(|s| {
                        let t = s.trim();
                        if t.is_empty() {
                ...
                // Build selectors map.
                let mut selectors: std::collections::HashMap<RelationshipType, String> =
                    std::collections::HashMap::new();
                if let Some(w) = weight_property.as_deref() {
                    let w = w.trim();
                    if !w.is_empty() {
                        for t in set.iter() {
                            selectors.insert(t.clone(), w.to_string());
                        }
                    }
                }
                for (k, v) in relationship_property_selectors.iter() {
                    let rk = k.trim();
                    let rv = v.trim();
                    if rk.is_empty() || rv.is_empty() {
                        continue;
                    }
                    selectors.insert(RelationshipType::of(rk), rv.to_string());
                }

                if selectors.is_empty() {
                    match store.get_graph_with_types(&set) {
                        Ok(g) => g,
                        Err(e) => {
                            return err(op, "ERROR", &format!("Failed to build graph view: {e}"))
                        }
                    }
                } else {
                    match store.get_graph_with_types_and_selectors(&set, &selectors) {
                        Ok(g) => g,
                        Err(e) => {
                            return err(op, "ERROR", &format!("Failed to build graph view: {e}"))
                        }
            ...
            let cfg_json = request.get("config").unwrap_or(&serde_json::Value::Null);
            let mut builder = PageRankConfig::builder();
            if let Some(mi) = cfg_json.get("maxIterations").and_then(|v| v.as_u64()) {
                builder = builder.max_iterations(mi as usize);
            }
            if let Some(tol) = cfg_json.get("tolerance").and_then(|v| v.as_f64()) {
                builder = builder.tolerance(tol);
            }
            if let Some(df) = cfg_json.get("dampingFactor").and_then(|v| v.as_f64()) {
                builder = builder.damping_factor(df);
            }
            let config = match builder.build() {
                Ok(c) => c,
                Err(e) => return err(op, "INVALID_CONFIG", &e.to_string()),
            };

            let result = run_pagerank(Arc::clone(&graph), config, None);

            let mut rows: Vec<serde_json::Value> = Vec::with_capacity(result.scores.len());
            for mapped_node_id in 0..(result.scores.len() as i64) {
                let original = graph
                    .to_original_node_id(mapped_node_id)
                    .unwrap_or(mapped_node_id);
                rows.push(serde_json::json!({
                    "nodeId": original,
                    "score": result.scores[mapped_node_id as usize],
                }));
            }
            ...
            ok(
                ...
                serde_json::json!({
                    "graphName": graph_name,
                    "rows": rows,
                    "ranIterations": result.ran_iterations,
