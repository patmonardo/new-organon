...
pub fn handle_node_similarity(request: &Value, catalog: Arc<dyn GraphCatalog>) -> Value {
    ...
    let parsed = match NodeSimilarityRequest::parse(request) {
        Ok(r) => r,
    ...
    match parsed.common.mode {
        ...
        Mode::Mutate => err(
            op,
            "NOT_IMPLEMENTED",
            "NodeSimilarity mutate is not implemented yet",
        ),
        Mode::Write => err(
            op,
            "NOT_IMPLEMENTED",
            "NodeSimilarity write is not implemented yet",
        ),
    }
}
...
fn run_stream(
    ...
    let task = Tasks::leaf("NodeSimilarity::stream".to_string())
        .base()
        .clone();
...
fn run_stats(op: &str, request: &NodeSimilarityRequest, graph_resources: &GraphResources) -> Value {
    let deps = RequestScopedDependencies::new(
        JobId::new(),
        TaskRegistryFactories::empty(),
        TerminationFlag::running_true(),
