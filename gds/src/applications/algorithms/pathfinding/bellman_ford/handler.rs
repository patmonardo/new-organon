use super::modes;
use super::request::BellmanFordRequest;
...
pub fn handle_bellman_ford(request: &Value, catalog: Arc<dyn GraphCatalog>) -> Value {
    ...
    let parsed = match BellmanFordRequest::parse(request) {
        Ok(r) => r,
    ...
    match parsed.common.mode {
        ...
        Mode::Mutate => err(
            op,
            "NOT_IMPLEMENTED",
            "Bellman-Ford mutate is not implemented yet",
        ),
        Mode::Write => err(
            op,
            "NOT_IMPLEMENTED",
            "Bellman-Ford write is not implemented yet",
        ),
    }
}
