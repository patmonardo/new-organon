//! DFS pathfinding algorithm dispatch handler.
//!
//! Handles JSON requests for DFS pathfinding operations,
//! delegating to the facade layer for execution.

use crate::procedures::facades::pathfinding::dfs::DfsBuilder;
use crate::types::catalog::GraphCatalog;
use serde_json::{json, Value};
use std::sync::Arc;

/// Handle DFS pathfinding requests
pub fn handle_dfs(request: &Value, catalog: Arc<dyn GraphCatalog>) -> Value {
    ...
    let targets = if let Some(target) = request.get("targetNode").and_then(|v| v.as_u64()) {
        ...
        vec![] // DFS can work without specific targets
    };
    ...
    // Create builder
    let mut builder = DfsBuilder::new(graph_store)
        .source(source)
