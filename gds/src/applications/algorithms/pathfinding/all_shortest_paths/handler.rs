use super::modes;
use super::request::AllShortestPathsRequest;
...
pub fn handle_all_shortest_paths(request: &Value, catalog: Arc<dyn GraphCatalog>) -> Value {
    ...
    let parsed = match AllShortestPathsRequest::parse(request) {
        Ok(r) => r,
    ...
    match parsed.common.mode {
        ...
        Mode::Mutate => err(op, "NOT_IMPLEMENTED", "AllShortestPaths mutate is not implemented yet"),
        Mode::Write => err(op, "NOT_IMPLEMENTED", "AllShortestPaths write is not implemented yet"),
    }
}
