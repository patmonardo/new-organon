...
use crate::applications::algorithms::pathfinding::dijkstra::steps::mutate_step::{
    DijkstraMutateOutcome, DijkstraMutateStep,
};
...
pub fn run(
    ...
    let task = Tasks::leaf("Dijkstra::mutate".to_string()).base().clone();

    let compute = move |gr: &GraphResources,
                        _tracker: &mut dyn ProgressTracker,
                        _termination: &TerminationFlag|
     -> Result<Option<DijkstraMutateOutcome>, String> {
    ...
    let builder = FnStatsResultBuilder(
        |_gr: &GraphResources, outcome: Option<DijkstraMutateOutcome>, timings| {
            json!({
                "ok": true,
                "op": op,
                "mode": "mutate",
