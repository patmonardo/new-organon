...
pub fn handle_yens(request: &Value, catalog: Arc<dyn GraphCatalog>) -> Value {
    ...
    let parsed = match YensRequest::parse(request) {
        Ok(r) => r,
    ...
    match parsed.common.mode {
        ...
        Mode::Mutate => err(op, "NOT_IMPLEMENTED", "Yen's mutate is not implemented yet"),
        Mode::Write => err(op, "NOT_IMPLEMENTED", "Yen's write is not implemented yet"),
    }
}
