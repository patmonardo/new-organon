//! Yen's K-shortest paths algorithm dispatch handler.
...
pub fn handle_yens(request: &Value, catalog: Arc<dyn GraphCatalog>) -> Value {
    ...
    let k = get_usize(request, "k")
        .or_else(|| get_usize(request, "maxPaths"))
        .unwrap_or(1);
    ...
    match mode {
        ...
        "stats" => {
            ...
            let builder = FnStatsResultBuilder(
                |_gr: &GraphResources, stats: Option<YensStats>, timings| {
                    json!({
                        "ok": true,
                        "op": op,
                        "mode": "stats",
