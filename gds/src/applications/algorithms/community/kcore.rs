...
use crate::procedures::community::kcore::KCoreFacade;
...
pub fn handle_kcore(request: &Value, catalog: Arc<dyn GraphCatalog>) -> Value {
    ...
    match mode {
        "stream" => {
            let task = Tasks::leaf("kcore::stream".to_string()).base().clone();
            ...
            match convenience.process_stream(
                ...
                Err(e) => err(op, "EXECUTION_ERROR", &format!("KCore stream failed: {e}")),
            }
        }
        "stats" => {
            let task = Tasks::leaf("kcore::stats".to_string()).base().clone();
