//! SCC algorithm dispatch handler.
//!
...
use crate::procedures::community::scc::SccFacade;
...
pub fn handle_scc(request: &Value, catalog: Arc<dyn GraphCatalog>) -> Value {
    ...
    match mode {
        "stream" => {
            let task = Tasks::leaf("scc::stream".to_string()).base().clone();
            ...
            match convenience.process_stream(
                ...
                Err(e) => err(op, "EXECUTION_ERROR", &format!("SCC stream failed: {e}")),
            }
        }
        "stats" => {
            let task = Tasks::leaf("scc::stats".to_string()).base().clone();
        ...
        "mutate" => {
            let property_name = request
                ...
                .unwrap_or_else(|| "componentId".to_string());

            let facade =
                SccFacade::new(Arc::clone(graph_resources.store())).concurrency(concurrency_value);
