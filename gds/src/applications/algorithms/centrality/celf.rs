...
use crate::procedures::centrality::celf::CELFFacade;
...
pub fn handle_celf(request: &Value, catalog: Arc<dyn GraphCatalog>) -> Value {
    ...
    match mode {
        "stream" => {
            let task = Tasks::leaf("celf::stream".to_string()).base().clone();

            let compute = |_gr: &crate::core::loading::GraphResources,
        ...
        "stats" => {
            let task = Tasks::leaf("celf::stats".to_string()).base().clone();
        ...
        "mutate" => {
            ...
            let facade = CELFFacade::new(Arc::clone(graph_resources.store()))
                .seed_set_size(seed_set_size)
                .monte_carlo_simulations(monte_carlo_simulations)
                .propagation_probability(propagation_probability)
                .batch_size(batch_size)
