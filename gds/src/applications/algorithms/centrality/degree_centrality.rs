...
pub fn handle_degree_centrality(request: &Value, catalog: Arc<dyn GraphCatalog>) -> Value {
    ...
    match mode {
        "stream" => {
            let stream_orientation = orientation;

            let task = Tasks::leaf("degree_centrality::stream".to_string())
                .base()
                .clone();
        ...
        "stats" => {
            let stats_orientation = orientation;

            let task = Tasks::leaf("degree_centrality::stats".to_string())
                .base()
                .clone();
