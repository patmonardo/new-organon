...
macro_rules! huge_collections {
    ...
    (
        ...
        impl $crate::collections::traits::Collections<$element_type> for $type_name {
            ...
            // Aggregations
            fn sum(&self) -> Option<$element_type>
            where
                $element_type: std::iter::Sum,
            {
                Some(self.iter().sum())
            }
            fn mean(&self) -> Option<f64> {
                if self.size() == 0 {
                    ...
                    Some(self.iter().map(|x| ($to_f64)(x)).sum::<f64>() / self.size() as f64)
                }
            }
            ...
            fn median(&self) -> Option<$element_type> {
                if self.size() == 0 {
                    ...
                    let mut v: Vec<$element_type> = self.iter().collect();
            ...
            fn percentile(&self, p: f64) -> Option<$element_type> {
                if self.size() == 0 || !(0.0..=100.0).contains(&p) {
                    None
                } else {
                    let mut v: Vec<$element_type> = self.iter().collect();
                    v.sort_by(|a, b| a.partial_cmp(b).unwrap());
    ...
    (
        ...
        impl $crate::collections::traits::Collections<$element_type> for $type_name {
            ...
            fn percentile(&self, _p: f64) -> Option<$element_type>
            where
                $element_type: Ord,
            {
                None
