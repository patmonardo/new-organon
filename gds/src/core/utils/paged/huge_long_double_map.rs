...
///
/// Essential for graph algorithms requiring node-to-score mappings:
/// - Node ID → PageRank scores
/// - Node ID → Centrality metrics
/// - Node ID → Embedding values
/// - Node ID → Probability/weight values
///
/// Uses +1 key shifting internally to distinguish empty slots (0) from key 0.
pub struct HugeLongDoubleMap {
    keys: HugeLongArray,
...
impl HugeLongDoubleMap {
    ...
    pub fn add_to(&mut self, key: i64, value: f64) {
        self.add_to0(key + 1, value);
    }

    /// Gets the value for a key, or returns default if not found.
    pub fn get_or_default(&self, key: i64, default_value: f64) -> f64 {
    ...
    fn add_to0(&mut self, key: i64, value: f64) {
    ...
    fn get_or_default0(&self, key: i64, default_value: f64) -> f64 {
        let hash = Self::mix_hash(key);
    ...
    fn rehash(&mut self, from_keys: &HugeLongArray, from_values: &HugeDoubleArray) {
...
impl Default for HugeLongDoubleMap {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn add_to_and_get() {
        ...
        assert!((map.get_or_default(1, 0.0) - 0.5).abs() < 1e-10);
        assert!((map.get_or_default(2, 0.0) - 1.5).abs() < 1e-10);
        assert!((map.get_or_default(3, 0.0) - 2.5).abs() < 1e-10);
        assert!((map.get_or_default(999, -1.0) - (-1.0)).abs() < 1e-10);
    }
    ...
    fn add_to_accumulates() {
        ...
        assert!((map.get_or_default(1, 0.0) - 0.6).abs() < 1e-10);
    }
    ...
    #[test]
    fn resize_on_growth() {
        let mut map = HugeLongDoubleMap::with_capacity(2);
        for i in 0..100 {
            map.add_to(i, i as f64 * 0.5);
        }

        for i in 0..100 {
            let expected = i as f64 * 0.5;
            assert!((map.get_or_default(i, -1.0) - expected).abs() < 1e-10);
        }
    }
    ...
    fn clear_works() {
        ...
        assert!(map.is_empty());
        assert!((map.get_or_default(1, -1.0) - (-1.0)).abs() < 1e-10);
    }
