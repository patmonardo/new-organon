...
pub trait ComputeSurface: Send + Sync {
	...
	fn allocate_suite(&self, request: ComputeSuiteRequest) -> Result<ComputeLease, ComputeSurfaceError>;
...
impl ComputeSurface for InMemoryComputeSurface {
	...
	fn allocate_suite(&self, request: ComputeSuiteRequest) -> Result<ComputeLease, ComputeSurfaceError> {
		...
		let gpu_candidates = guard
			...
			.filter(|d| request.require_labels.iter().all(|(k, v)| d.labels.get(k) == Some(v)))
...
mod tests {
	...
	fn allocates_exclusive_gpu_suite() {
		let surface = InMemoryComputeSurface::with_devices(vec![gpu(1, 80_000, "nvidia"), gpu(2, 80_000, "nvidia")]);
