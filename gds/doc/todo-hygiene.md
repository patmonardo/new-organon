...
- Format: `TODO(owner,YYYY-MM-DD): message` (owner = handle/team; date = target or reevaluation).
- Scope: `gds/src/**` and runtime code. Docs (`gds/doc/**`) and Markdown are excluded from the check.
- Script: `tools/check-gds-todos.sh` (defaults to changed GDS files; pass paths to override). Fails on bare `TODO`.
