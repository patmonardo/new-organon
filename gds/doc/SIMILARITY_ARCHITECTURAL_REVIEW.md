...
- **Completeness**: The config surface now includes the main NN-Descent knobs (`max_iterations`, `sampled_k`, `initial_sampler`, `random_seed`, `perturbation_rate`, `random_joins`, `update_threshold`). What’s still missing versus Java is parity-level task partitioning semantics (e.g. min batch sizing), neighbor-consumer instrumentation, and exposing convergence/iteration stats in the public Rust result types.
